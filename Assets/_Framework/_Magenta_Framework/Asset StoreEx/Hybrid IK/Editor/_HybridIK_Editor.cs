...
[CustomEditor(typeof(_HybridIK))]
public class _HybridIK_Editor : HybridIKNodeInspector
{
    private static string LOG_FORMAT = "<color=#00FFA5><b>[_HybridIK_Editor]</b></color> {0}";

    // protected SerializedProperty m_Script;

    protected _HybridIK _hybridIK 
    { 
        get 
        { 
            return target as _HybridIK;
        }
    }
    ...
    protected virtual void AddJointsPoseKeyframe()
    {
        List<int> keyframeState = new List<int>();
        foreach (HybridIKJoint node in _hybridIK.nodeList)
        {
            _HybridIKJoint _node = node as _HybridIKJoint;

            // keyframeState.Add(j.enableKeyframeConstraints ? 0 : j.overrideConstraint ? 1 : 2);
            if (_node.enableKeyframeConstraints == true)
            {
                keyframeState.Add(0);
            }
            else
            {
                if (_node.overrideConstraint == true)
                {
                    keyframeState.Add(1);
                }
                else
            ...
            _node.enableKeyframeConstraints = true; _node.overrideConstraint = false;
        }

        _hybridIK.ReprocessJoints();

        _HybridIKJoint root = _hybridIK.nodeList[0] as _HybridIKJoint;
        Transform parent = root.jointTransform.parent;

        foreach (HybridIKJoint _node in _hybridIK.nodeList)
        {
            _HybridIKJoint node = _node as _HybridIKJoint;
            ...
            if (parent != null)
            {
                node.keyedPositions.Add(parent.InverseTransformPoint(node.GetCurrentPositionWorld()));
            }
            else
            {
                node.keyedPositions.Add(node.GetCurrentPositionWorld());
            }

            HybridIKConstraint constraint = _hybridIK.constraints.Find(c => c.jointTransform == node.jointTransform);
            if (constraint == null)
            {
                constraint = new HybridIKConstraint();
                constraint.jointTransform = node.jointTransform;
                constraint.constrainPosition = true;
                _hybridIK.constraints.Add(constraint);
            }
        }

        foreach (HybridIKConstraint c in _hybridIK.constraints)
        {
            ...
            Vector3 endPos = parent != null ? parent.InverseTransformPoint(_hybridIK.endNode.position) : _hybridIK.endNode.position;
            key.SetEndTargetPosition(_hybridIK.endNode.position, parent);
            c.positionKeys.Add(key);
        }
        _hybridIK.ResetToZeroPose();
    }

    protected virtual void HandleSelectedIKNodeIndex()
    {
        GUIStyle richTextLabelStyle = new GUIStyle(GUI.skin.label);
        richTextLabelStyle.richText = true;

        HybridIKJoint _selectedNode = _hybridIK.nodeList[selectedIKNodeIndex];
        ...
        if (selectedIKNodeIndex >= 0)
        {
            ...
            if (GUI.changed)
                EditorUtility.SetDirty(_hybridIK);
            return;
        }
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        */
        ...
        if (_hybridIK.nodeList == null)
            return;

        if (_hybridIK.constraints == null || _hybridIK.constraints.Count == 0)
        {
            _hybridIK.constraints = new List<HybridIKConstraint>();
            foreach (HybridIKJoint node in _hybridIK.nodeList)
            {
                node.keyedLocalPositions = new List<Vector3>();
                node.keyedPositions = new List<Vector3>();
        ...
        if (_hybridIK.constraints != null)// && hybridIKNode.constraints.Count > 0)
        {
            ...
            if (_hybridIK.nodeList != null && _hybridIK.nodeList.Count > 0 && 
                _hybridIK.nodeList[0] != null && _hybridIK.nodeList[0].keyedPositions != null)
            {
                for (int p = 0; p < _hybridIK.nodeList[0].keyedPositions.Count; ++p)
                {
                    GUILayout.BeginHorizontal();
    ...
    public override void OnSceneGUI()
    {
        ...
        if (_hybridIK.IsInitialized())
        {
            for (int i = 0; i < _hybridIK.nodeList.Count; ++i)
            //foreach (SweepIKNode n in hybridIKNode.nodes)
            {
                HybridIKJoint n = _hybridIK.nodeList[i];
                // Color c = n.enableKeyframeConstraints ? Color.blue : n.overrideConstraint ? Color.cyan : originalColor;
                Color c = Color.blue;
                if (n.enableKeyframeConstraints == false)
                {
                    if (n.overrideConstraint == true)
                    {
                        c = Color.cyan;
                    }
                    else
                ...
                if (Handles.Button(n.GetCurrentPositionWorld(), Quaternion.identity, nRadius, nRadius, Handles.SphereHandleCap))
                {
                    ...
                    Debug.LogFormat(LOG_FORMAT, "Button Clicked!!!!!!!!!!!!!!!!, selectedIKNodeIndex : <b>" + selectedIKNodeIndex + "</b>");
                    selectedIKKeyframe = -1;
            ...
            _HybridIKJoint root = _hybridIK.GetRootIKNode() as _HybridIKJoint;
            Debug.AssertFormat(root != null, "_hybridIK.nodeList.Count : " + _hybridIK.nodeList.Count);
            _HybridIKJoint endIKNode = _hybridIK.GetEndIKNode() as _HybridIKJoint;
            Debug.Assert(endIKNode != null);
                ...
                if (c.positionKeys.Count > 0)
                {
                    for (int i = 0; i < c.positionKeys.Count; ++i)
                    {
                        ...
                        if (Handles.Button(buttonPos, Quaternion.identity, endIKNode.jointRadius, endIKNode.jointRadius, Handles.SphereHandleCap))
                        {
                            ...
                            _hybridIK.PoseToKeyframe(selectedIKKeyframe);
                            Repaint();
                        }
                    }
                }

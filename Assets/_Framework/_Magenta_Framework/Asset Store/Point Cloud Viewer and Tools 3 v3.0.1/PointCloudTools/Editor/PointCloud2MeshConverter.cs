...
namespace pointcloudviewer.pointcloud2mesh
{
    ...
    {
        protected static string appName = "PointCloud2Mesh";
        private Object sourceFile;
        private string externalSourceFile;
        string lastUsedExternalFolder = null;
        ...
        private bool removeEveryNthPoint = false;
        private int removeEveryNth = 2;
        private bool keepEveryNthPoint = false;
        private int keepEveryNth = 5;
        ...
        void OnGUI()
        {
            ...
            externalSourceFile = EditorGUILayout.TextField("Or use external file:", externalSourceFile);
            if (GUILayout.Button(new GUIContent("...", "Browse for external point cloud file"), GUILayout.Height(16), GUILayout.Width(22)))
            {
                externalSourceFile = EditorUtility.OpenFilePanelWithFilters("Select point cloud file", lastUsedExternalFolder, new string[] { "Point cloud files", "xyz,xyzrgb,cgo,asc,ply,pts,pcd,las", "All files", "*" });
                if (string.IsNullOrEmpty(externalSourceFile) == false)
                {
                    // clear local file reference
                    sourceFile = null;

                    // save this folder as default for next time
                    var lastUsedExternalFolderTemp = Path.GetDirectoryName(externalSourceFile);
                    if (string.IsNullOrEmpty(lastUsedExternalFolderTemp) == false)
                    {
                        lastUsedExternalFolder = lastUsedExternalFolderTemp;
                        EditorPrefs.SetString(prefsPrefix + "defaultExternalFolder", lastUsedExternalFolder);
                    }

                    Debug.Log("externalSourceFile= " + externalSourceFile);
                }
            }
            ...
            if (sourceFile != null)
            {
                // if local file is assigned, clear external file
                externalSourceFile = null;
                ...
                {
                    ShowHeaderDataHelper.PrintHeaderData(sourceFile);
                }
            }
            else // local source file is null
            {
                // if external file is set, display info about file
                if (string.IsNullOrEmpty(externalSourceFile) == false)
                {
                    GUILayout.Label("File:" + GetExternalFileInfo(), EditorStyles.miniLabel);
                    if (GUILayout.Button(new GUIContent("Show Header", "Prints few rows from the file into console (to check values)"), GUILayout.Height(16)))
                    {
                        ShowHeaderDataHelper.PrintExternalHeaderData(externalSourceFile);
                    }
                }
            ...
            removeEveryNthPoint = EditorGUILayout.ToggleLeft(new GUIContent("Decimate points (Remove)", null, "Skip rows from data (You should do this with external tools to get better results)"), removeEveryNthPoint);
            GUI.enabled = removeEveryNthPoint;
            removeEveryNth = EditorGUILayout.IntField(new GUIContent("Remove every Nth point", null, ""), Mathf.Clamp(removeEveryNth, 2, 999999));
            GUI.enabled = true;

            keepEveryNthPoint = EditorGUILayout.ToggleLeft(new GUIContent("Decimate points (Keep)", null, "Skip rows from data (You should do this with external tools to get better results)"), keepEveryNthPoint);
            GUI.enabled = keepEveryNthPoint;
            keepEveryNth = EditorGUILayout.IntField(new GUIContent("Keep every Nth point", null, ""), Mathf.Clamp(keepEveryNth, 2, 999999));
            GUI.enabled = true;
            ...
            EditorGUILayout.Space();
            // disabled if no source selected
            GUI.enabled = (sourceFile == null && string.IsNullOrEmpty(externalSourceFile)) ? false : true;
            if (GUILayout.Button(new GUIContent("Convert to Meshes", "Convert source to meshes"), GUILayout.Height(40)))
            {
                ...
                { // single point cloud
                    ...
                    if (sourceFile == null)
                    {
                        folder = new GameObject("PointClouds-" + Path.GetFileNameWithoutExtension(externalSourceFile));
                    }
                    else
                    {
                    ...
                    stopwatch.Stop();
                    Debug.Log("Convert2Mesh Timer: " + stopwatch.ElapsedMilliseconds + "ms");
                    stopwatch.Reset();
                }
        ...
        void Convert2Mesh(string fileToRead)
        {
            ...
            using (StreamReader reader = new StreamReader(File.OpenRead(fileToRead)))
            {
                ...
                for (rowCount = 0; rowCount < masterPointCount - 1; rowCount++)
                {
                    ...
                    {
                        if (!double.TryParse(lineSplitted[0], NumberStyles.Any, CultureInfo.InvariantCulture, out x)) skipRow = true;
                        if (!double.TryParse(lineSplitted[1], NumberStyles.Any, CultureInfo.InvariantCulture, out y)) skipRow = true;
                        if (!double.TryParse(lineSplitted[2], NumberStyles.Any, CultureInfo.CurrentCulture, out z)) skipRow = true;
                        //if (!double.TryParse(lineSplitted[0], out x)) skipRow = true;
                        //if (!double.TryParse(lineSplitted[1], out y)) skipRow = true;
                        //if (!double.TryParse(lineSplitted[2], out z)) skipRow = true;
                ...
                if (skippedRows > 0) Debug.LogWarning("Skipped " + skippedRows.ToString() + " rows (out of " + masterPointCount + " rows) because of parsing errors: " + fileToRead);
            ...
            if (splitToGrid == true)
            {
                ...
                {
                    if (removeEveryNthPoint == true)
                    {
                        // is now its this count
                        if (i % removeEveryNth == 0) continue;
                    }

                    if (keepEveryNthPoint == true)
                    {
                        // if not this count yet
                        if (i % keepEveryNth != 0) continue;
                    }
            ...
            else // old way
            {
                ...
                for (int i = 0, totalLen = vertexArray.Length; i < totalLen; i++)
                {
                    ...
                    {
                        // increment only if keep this
                        if (keepEveryNthPoint == true)
                        {
                            if (i % keepEveryNth == 0) indexCount++;
                        }
                        else
                        {
                            // take all
        ...
        void SaveMeshesToProject()
        {
            ...
            if (addMeshesToScene == false || createLODS == true)
            {
                ...
                for (int i = 0; i < cloudList.Count; i++)
                {
                    ...
                    if (i < 100) pad = "00";
                    if (i < 10) pad = "000";
                    // TODO create prefabs, instead of mesh only
            ...
            else // they are in scene
            {
                for (int i = 0; i < folder.transform.childCount; i++)
                {
                    ...
                    string fullSavePath = savePath + "_" + pcCounter + "_" + pad + i + ".prefab";
                    string MeshSavePath = "mesh_" + savePath + "_" + pcCounter + "_" + pad + i + ".asset";

                    // add mesh to gameobject first, otherwise issues with SaveAsPrefabAsset returning null
                    var mf = folder.transform.GetChild(i).gameObject.GetComponent<MeshFilter>();
                    mf.sharedMesh = cloudList[i];

#if UNITY_2018_4_OR_NEWER || UNITY_2019_1_OR_NEWER
                    // need to delete old one first, or get duplicated data
                    if (File.Exists(fullSavePath))
                    {
                        AssetDatabase.DeleteAsset(fullSavePath);
                    }

                    var rootPrefab = PrefabUtility.SaveAsPrefabAsset(folder.transform.GetChild(i).gameObject, fullSavePath);
                    var mesh = new Mesh();
                    mesh = mf.sharedMesh;
                    mesh.name = "mesh_" + savePath + "_" + pcCounter + "_" + pad + i;
                    AssetDatabase.AddObjectToAsset(mesh, fullSavePath);
                    rootPrefab.GetComponent<MeshFilter>().sharedMesh = mesh;
#else
                    var rootPrefab = PrefabUtility.CreatePrefab(fullSavePath, folder.transform.GetChild(i).gameObject);
#endif
        ...
        void BrekelDataConvert(string fileToRead)
        {
			...
			*/
            ...
            // TODO too many verts for empty/small meshes
            ...
            // FIXME first frame is wrong, contains points (shouldnt have for owl)
            // TODO skip 0 vert frames?

            for (int frame = 0; frame < numberOfFrames; frame++)
            {
                int MaxVertexCountPerMesh = numberOfPointsPerFrame[frame];

                Debug.Log("MaxVertexCountPerMesh=" + MaxVertexCountPerMesh);
            ...
            EditorUtility.DisplayProgressBar(appName, "Saving created meshes (total frames = "+numberOfFrames+")", 0.75f);
        ...
        GameObject BuildMesh(Vector3[] verts, int[] tris, Color[] colors, Vector3[] normals, Bounds? bounds = null)
        {
            ...
            // disable lightmap static (causes warning spam) also static batching need to be disabled (2019.4 and later?)
#if UNITY_2019_2_OR_NEWER
            GameObjectUtility.SetStaticEditorFlags(targetGO, ~StaticEditorFlags.ContributeGI & ~StaticEditorFlags.BatchingStatic);
#else
        ...
        void BuildLODS(GameObject mainGO, Vector3[] verts, int[] tris, Color[] colors, Vector3[] normals, Bounds? bounds = null)
        {
            ...
            for (int i = 0; i < lodLevels; i++)
            {
                ...
                else // create lod meshes
                {
                    ...
                    lerpVal -= lerpStep;
                    int newVertCount = (int)Mathf.Lerp(minLodVertexCount, removeEveryNthPoint ? MaxVertexCountPerMesh / removeEveryNth : MaxVertexCountPerMesh, lerpVal);
        ...
        string GetExternalFileInfo()
        {
            if (File.Exists(externalSourceFile))
            {
                string tempFileName = Path.GetFileName(externalSourceFile);
                return tempFileName + " (" + (new FileInfo(externalSourceFile).Length / 1000000) + "MB)";
            }
            return "";
        }
        ...
        void SavePreferences()
        {
            ...
            EditorPrefs.SetBool(prefsPrefix + "removeEveryNthPoint", removeEveryNthPoint);
            EditorPrefs.SetBool(prefsPrefix + "keepEveryNthPoint", keepEveryNthPoint);
            EditorPrefs.SetInt(prefsPrefix + "removeEveryNth", removeEveryNth);
            EditorPrefs.SetInt(prefsPrefix + "keepEveryNth", keepEveryNth);
        ...
        void LoadPreferences()
        {
            ...
            removeEveryNthPoint = EditorPrefs.GetBool(prefsPrefix + "removeEveryNthPoint", removeEveryNthPoint);
            keepEveryNthPoint = EditorPrefs.GetBool(prefsPrefix + "keepEveryNthPoint", keepEveryNthPoint);
            removeEveryNth = EditorPrefs.GetInt(prefsPrefix + "removeEveryNth", removeEveryNth);
            keepEveryNth = EditorPrefs.GetInt(prefsPrefix + "keepEveryNth", keepEveryNth);

...
namespace pointcloudviewer.pointcloud2binary
{
    ...
    {
        protected static string appName = "PointCloud2Binary";
        ...
        */
        ...
        void OnGUI()
        {
            ...
            gridSize = Mathf.Clamp(EditorGUILayout.FloatField(new GUIContent("Grid Cell Size", null, "Split cloud into tiles (size x size x size)"), gridSize), 0.01f, 1000);
        ...
        void ConvertBinaryV1()
        {
            ...
            if (sourceFile == null)
            {
                fileName = Path.GetFileName(externalSourceFile);
                fileToRead = externalSourceFile;
            }
            else
            {
                fileName = sourceFile.name;
                fileToRead = AssetDatabase.GetAssetPath(sourceFile);
            }

            saveFilePath = EditorUtility.SaveFilePanelInProject("Output binary file (v1)", fileName + ".bin", "bin", "");
                                ...
                                case "PCD (ASCII)": // PCD (ASCII)
                                    ...
                                    if (readRGB == true)
                                    {
                                        ...
                                        if (row.Length == 4)
                                        {
                                            ...
                                            double val = double.Parse(row[3], System.Globalization.NumberStyles.Any, CultureInfo.InvariantCulture);
                                            Debug.Log("row=" + row[3] + " val=" + val);

                                            //DoubleinBinaereundHexa(val);

                                            // double to raw 8 bytes
                                            ...
                                            {
                                                ss += bytes[i].ToString("X") + " ";
                                                string yourByteString = System.Convert.ToString(bytes[i], 2).PadLeft(8, '0');
                                                //string yourByteString = System.Convert.ToString(bytes[i], 2);
                                                by += yourByteString;
                                            }

                                            Debug.Log("raw:" + ss + " bytes len=" + bytes.Length);
                                            Debug.Log("raw:" + by);

                                            //int rr = System.BitConverter.ToInt16(bytes, 0)>>8;
                                            //int gg = System.BitConverter.ToInt32(bytes, 0);
                                            //int bb = System.BitConverter.ToInt32(bytes, 0);
                                            int rr = (byte)bytes[0] >> 8 + ((byte)bytes[1]);
                                            int gg = (byte)bytes[2];
                                            int bb = (byte)bytes[4];


                                            //int rrr = BytesToInt(bytes, 0);
                                            //int ggg = BytesToInt(bytes, 4);
                                            Debug.Log("rr= " + rr);
                                            //Debug.Log("gg= " + gg + "  ggg=" + ggg);

                                            //Debug.Log("long0= " + rrr + " int=" + ((int)rrr)+" byte="+((rrr >> 16) & 0x0000ff));
                                            //long ggg = System.BitConverter.ToInt64(bytes, 8);
                                            //Debug.Log("long1= " + ggg+ " int=" + ((int)ggg)+" byte="+((ggg >> 8) & 0x0000ff));

                                            long rgb = (long)val;
                                            //long rgb = System.BitConverter.ToInt32(bytes, 0);
                                            Debug.Log("long=" + rgb);

                                            bytes = System.BitConverter.GetBytes(rgb);
                                            ss = "";
                                            by = "";
                                            for (int i = 0; i < bytes.Length; i++)
                                            {
                                            ...
                                            Debug.Log("new:" + ss + " bytes len=" + bytes.Length);
                                            Debug.Log("new:" + by);

                                            //Debug.Log("rgb=" + rgb + " : " + (rgb & 0x000000ffffff));
                                            ...
                                            // BUG my values are half from real values
                                            // CC colors: 90,93,74
                                            // CC colors: 128,128,224
                                            Debug.Log(r + ", " + g + ", " + b);
        ...
        void ConvertBinaryV2()
        {
            ...
            // get input file path, from local reference or from external path
            if (sourceFile == null)
            {
                fileName = Path.GetFileName(externalSourceFile);
                fileToRead = externalSourceFile;
            ...
            if (useBinFormatV2 == false)
            {
                ...
                {
                    saveFilePath = EditorUtility.SaveFilePanelInProject("Output folder for binary v3 tiles", fileName, "pcroot", "");
                ...
                {
                    saveFilePath = EditorUtility.SaveFilePanelInProject("Output pCache", fileName + ".pcache", "pcache", "");
                }
                else
                {
                    saveFilePath = EditorUtility.SaveFilePanelInProject("Output binary v1 file", fileName + ".bin", "bin", "");
                }
            }
            else
            {
                saveFilePath = EditorUtility.SaveFilePanelInProject("Output binary v2 file", fileName + ".ucpc", "ucpc", "");
            }
            ...
            //using (BufferedStream streamReader = new BufferedStream(fs))
            {
                ...
                else if (useBinFormatV3 == true) // v3
                {
                    ...
                    // TEMP fix, replace commas with dots
                    for (int i = 0; i < tilerootdata.Count; i++)
                    {
                        tilerootdata[i] = tilerootdata[i].Replace(",", ".");
                    }
        ...
        int BytesToInt(byte[] value, int startIndex)
        {

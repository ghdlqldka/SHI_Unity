Shader "UnityCoder/PointCloud/DX11/ColorSizeV2-packed-lite"
{
	Properties
	{
		...
		SubShader
	{
		Pass
		{
			...
			StructuredBuffer<Point> buf_Points; // 4+4+4 bytes
			...
			void GS_Main(point GS_INPUT p[1], inout TriangleStream<FS_INPUT> triStream)
			{
				...
				// top
				newVert.pos = vertPos + float4(0,height,0,0);
				#ifdef _CIRCLE_ON
				newVert.uv = float2(0.5,1);
				#endif
				triStream.Append(newVert);

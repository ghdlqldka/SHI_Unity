// RGB+INT+CLASSIFICATION+XYZ Packed v3 format

Shader "UnityCoder/PointCloud/DX11/ColorSizeV3-Int-Class-Packed"
{
	Properties
	{
		...
		[HideInInspector] // because they are controlled by the keywords from Features script
		[KeywordEnum(Off, On)] _UseIntensity ("Use Intensity Color",float) = 1
		[HideInInspector]
		[KeywordEnum(Off, On)] _UseClassification ("Use Classification Color",float) = 1
		[HideInInspector]
	...
	SubShader
	{
		Pass
		{
			...
			#pragma multi_compile _USEINTENSITY_OFF _USEINTENSITY_ON
			#pragma multi_compile _USECLASSIFICATION_OFF _USECLASSIFICATION_ON
			...
			// returns float3(color, scalar, pos)
			...
			void GS_Main(point GS_INPUT po[1], inout TriangleStream<FS_INPUT> triStream)
			{
				...
				float3 xcr = UnpackValues(asuint(rawpos.x));
				float3 giy = UnpackValues(asuint(rawpos.y));
				float2 zb = SuperUnpacker(rawpos.z);
				float3 p = float3(xcr.z + _Offset.x, giy.z + _Offset.y, zb.y + _Offset.z);
				fixed3 col = 0;

				#if defined(_USEINTENSITY_ON)
				float ii = giy.y;
				col = ii.xxx;
				#endif
				
				#if defined(_USECLASSIFICATION_ON)
				float ii2 = xcr.y;
				col = ii2.xxx;
				#endif
				
				#ifdef _GRADIENT_ON
				col = tex2Dlod(_GradientTex, float4(col.x,0.5f,0,0)).rgb;
				#else
				#if defined(_USEINTENSITY_OFF) && defined(_USECLASSIFICATION_OFF)
				col = fixed3(saturate(xcr.x), saturate(giy.x), saturate(zb.x)) * 1.01;
				#endif
				#endif

				#if !UNITY_COLORSPACE_GAMMA
				//#if _USEINTENSITY_OFF && _USECLASSIFICATION_OFF
				#if _GRADIENT_OFF
				col = col * col;
				#endif
				#endif

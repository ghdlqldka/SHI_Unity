...
namespace PointCloudRuntimeViewer
{
    ...
    public class RuntimeViewerDX11 : MonoBehaviour
    {
        ...
        protected CommandBuffer commandBufferDepth;
        ...
        public virtual void CallImporterThreaded(string fullPath)
        {
        ...
        public virtual void LoadRawPointCloud(System.Object a)
        {
            // cleanup old buffers
            ...
            using (StreamReader streamReader = new StreamReader(File.OpenRead(fullPath)))
            {
                        ...
                        if (row.Length > 2)
                        {
                            switch (pointCloudFormat)
                            {
                                ...
                                case PointCloudFormat.XYZ: // XYZ RGB(INT)
                                    ...
                                    //y = double.Parse(row[1], CultureInfo.InvariantCulture);
                                    //z = double.Parse(row[2], CultureInfo.InvariantCulture);
                                    if (double.TryParse(row[0], NumberStyles.Any, CultureInfo.InvariantCulture, out x) == false) { skippedRows++; continue; };
                                    if (double.TryParse(row[1], NumberStyles.Any, CultureInfo.InvariantCulture, out y) == false) { skippedRows++; continue; };
                                    if (double.TryParse(row[2], NumberStyles.Any, CultureInfo.InvariantCulture, out z) == false) { skippedRows++; continue; };
                                    if (readRGB == true)
                                    {
                                        ...
                                        if (int.TryParse(row[3], NumberStyles.Any, CultureInfo.InvariantCulture, out int rInt) == false) { skippedRows++; continue; };
                                        if (int.TryParse(row[4], NumberStyles.Any, CultureInfo.InvariantCulture, out int gInt) == false) { skippedRows++; continue; };
                                        if (int.TryParse(row[5], NumberStyles.Any, CultureInfo.InvariantCulture, out int bInt) == false) { skippedRows++; continue; };
                                        r = LUT255[rInt];
                                        g = LUT255[gInt];
                                        b = LUT255[bInt];
                                    }
                ...
                MainThread.Call(OnLoadingCompleteCallBack,fullPath);
        ...
        public virtual void InitDX11Buffers()
        {
        ...
        protected void ReleaseDX11Buffers()
        {
        ...
        protected virtual void OnLoadingCompleteCallBack(System.Object a)
        {
        ...
        protected virtual void SelectClosestPoint()
        {
            // left click for measuring
            if (Input.GetMouseButtonDown(0))
            {
                if (!isSearchingPoint && hasLoadedPointCloud) StartCoroutine(FindClosestPointBrute(Input.mousePosition));
            }
        }
        ...
        protected float Remap(float source, float sourceFrom, float sourceTo, float targetFrom, float targetTo)
        {
            return targetFrom + (source - sourceFrom) * (targetTo - targetFrom) / (sourceTo - sourceFrom);
        }

        protected int CharCount(string source, char separator)
        {
            int count = 0;
        ...
        protected bool isBuildingMesh = false;
        int meshCounter = 0;
        protected void BuildMesh(System.Object a)
        {
            var m = (TempMesh)a;
            var verts = m.verts;
            var tris = m.tris;

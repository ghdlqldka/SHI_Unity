...
{
    public class MeshBoxSelectExample : MonoBehaviour
    {
        public MeshPointsBoxSelect meshPointsBoxSelect;
        public Text measureInfoText;
        public Renderer selectionBox;

        [Tooltip("Hide selected points by moving them away")]
        public bool hideSelectedPoints = true;

        protected Camera cam;

        protected bool isTesting = false;

        protected List<Vector3> verticesTemp = new List<Vector3>();
        List<Color> colorsTemp = new List<Color>();

        protected GameObject cachedMeshGo = null;
        protected Mesh cachedMesh = null;
        ...
        protected virtual void Update()
        {
            ...
            if (Input.GetMouseButtonDown(0) || isTesting)
            {
                ...
                // call point selector
                meshPointsBoxSelect.Select(selectionBox.GetComponent<BoxCollider>());
        ...
        public virtual void OnPointsPicked(List<PointsData> selectedPoints)
        {
            Debug.Log($"OnPointsPicked, found: {selectedPoints.Count} points");
            if (measureInfoText != null) measureInfoText.text = $"Found {selectedPoints.Count} points";
        ...
        public virtual void OnPointsNotFound()
        {

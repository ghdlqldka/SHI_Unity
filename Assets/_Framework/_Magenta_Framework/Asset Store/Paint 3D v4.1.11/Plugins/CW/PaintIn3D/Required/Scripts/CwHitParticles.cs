...
namespace PaintIn3D
{
	...
	public class CwHitParticles : MonoBehaviour
	{
		...
		/// <summary>Hit events are normally sent to all components attached to the current GameObject, but this setting allows you to override that. This is useful if you want to use multiple <b>CwHitParticles</b> components with different settings and results.</summary>
		...
		[System.NonSerialized]
		protected bool cachedParticleSystemSet;

		[System.NonSerialized]
		protected static List<ParticleCollisionEvent> particleCollisionEvents = new List<ParticleCollisionEvent>();
		...
		protected bool TryGetRaycastHit(ParticleCollisionEvent collision, ref RaycastHit hit)
		{
			...
			for (var i = 0; i < count; i++)
			{
				...
				switch (emit)
				{
					...
					case EmitType.PointsOnUV:
					{
						...
						{
							hitCache.InvokeCoord(finalRoot, preview, priority, finalPressure, new CwHit(hit), finalRotation);
						}
					}
					break;
					...
					case EmitType.TrianglesIn3D:
					{
						...
						if (TryGetRaycastHit(collision, ref hit) == true)
						{
							hitCache.InvokeTriangle(gameObject, preview, priority, finalPressure, new CwHit(hit), finalRotation);
						}
					}
					break;
				}
...
{
	using UnityEditor;
	using TARGET = CwHitParticles;

	[CustomEditor(typeof(TARGET))]
	public class CwHitParticles_Editor : CwEditor
	{

...
namespace PaintIn3D
{
	...
	public class CwPaintableMesh : CwMeshModel
	{
		...
		public List<Renderer> OtherRenderers { set { otherRenderers = value; } get { if (otherRenderers == null) otherRenderers = new List<Renderer>(); return otherRenderers; } } 
		[SerializeField] protected List<Renderer> otherRenderers;
		...
		protected HashSet<CwPaintableMeshTexture> paintableTextures = new HashSet<CwPaintableMeshTexture>();

		[System.NonSerialized]
		private List<Mesh> tempDilateMeshes;

		[System.NonSerialized]
		protected static List<CwMaterialCloner> tempMaterialCloners = new List<CwMaterialCloner>();

		[System.NonSerialized]
		protected static List<CwPaintableTexture> tempPaintableTextures = new List<CwPaintableTexture>();

		[System.NonSerialized]
		protected static List<CwPaintableMeshTexture> tempPaintableMeshTextures = new List<CwPaintableMeshTexture>();
		...
		protected virtual void AddPaintableTextures(Transform root)
		{
		...
		public List<Mesh> GetDilateMeshes()
		{
			if (tempDilateMeshes == null)
			{
				...
				TryPopulateDilateMeshes(CachedRenderer);

				if (otherRenderers != null)
				{
					foreach (var renderer in otherRenderers)
					{
						TryPopulateDilateMeshes(renderer);
					}
				}
			}
		...
		private void TryPopulateDilateMeshes(Renderer renderer)
		{
			if (renderer != null)
			{
				...
				{
					tempDilateMeshes.Add(meshFilter.sharedMesh);
				}
			}
		}
		...
		public virtual void ClearAll(Texture texture, Color color)
		{
			if (activated == true)
			{
				foreach (var paintableTexture in paintableTextures)
		...
		public virtual void Register(CwPaintableMeshTexture paintableTexture)
		{
			Debug.Log("#################################");
			paintableTextures.Add(paintableTexture);
		}
		...
		public virtual void Unregister(CwPaintableMeshTexture paintableTexture)
		{

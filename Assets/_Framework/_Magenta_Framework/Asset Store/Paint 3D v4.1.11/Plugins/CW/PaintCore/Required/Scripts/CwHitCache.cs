...
namespace PaintCore
{
	...
	public class CwHitCache
	{
		...
		[System.NonSerialized]
		protected List<IHitPoint> hitPoints = new List<IHitPoint>();

		[System.NonSerialized]
        protected List<IHitLine> hitLines = new List<IHitLine>();

		[System.NonSerialized]
        protected List<IHitTriangle> hitTriangles = new List<IHitTriangle>();

		[System.NonSerialized]
        protected List<IHitQuad> hitQuads = new List<IHitQuad>();

		[System.NonSerialized]
        protected List<IHitCoord> hitCoords = new List<IHitCoord>();

		[System.NonSerialized]
		protected static List<IHit> hits = new List<IHit>();
		...
		public virtual void InvokePoint(GameObject gameObject, bool preview, int priority, float pressure, Vector3 position, Quaternion rotation)
		{
			if (cached == false)
			{
				Cache(gameObject);
		...
		public void InvokeTriangle(GameObject gameObject, bool preview, int priority, float pressure, CwHit hit, Quaternion rotation)
		{
			...
			if (CwMeshCache.GetTrianglePositions(hit, ref positionA, ref positionB, ref positionC) == true)
			{
		...
		protected virtual void Cache(GameObject gameObject)
		{
			cached = true;

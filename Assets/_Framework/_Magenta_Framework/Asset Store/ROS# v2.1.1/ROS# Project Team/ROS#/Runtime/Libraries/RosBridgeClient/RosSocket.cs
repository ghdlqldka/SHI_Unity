...
namespace RosSharp.RosBridgeClient
{
    public class RosSocket
    {
        ...
        protected readonly Dictionary<SerializerEnum, ISerializer> serializerDictionary = new Dictionary<SerializerEnum, ISerializer>()
        {
            { SerializerEnum.Microsoft, new MicrosoftSerializer()},
            { SerializerEnum.Newtonsoft_JSON, new NewtonsoftJsonSerializer()}
        };
        protected Dictionary<string, Publisher> Publishers = new Dictionary<string, Publisher>();
        protected Dictionary<string, Subscriber> Subscribers = new Dictionary<string, Subscriber>();
        protected Dictionary<string, ServiceProvider> ServiceProviders = new Dictionary<string, ServiceProvider>();
        protected Dictionary<string, ServiceConsumer> ServiceConsumers = new Dictionary<string, ServiceConsumer>();

#if ROS2
        protected Dictionary<string, ActionProvider> ActionProviders = new Dictionary<string, ActionProvider>();
        protected Dictionary<string, ActionConsumer> ActionConsumers = new Dictionary<string, ActionConsumer>();
#endif
        ...
        public virtual void Close(int millisecondsWait = 0)
        {
#if ROS2
        ...
        public virtual string Advertise<T>(string topic) where T : Message
        {
            string id = topic;
        ...
        public virtual void Publish(string id, Message message)
        {
            Send(Publishers[id].Publish(message));
        }

        public virtual void Unadvertise(string id)
        {
...
#endif
        protected void Send<T>(T communication) where T : Communication
        {
        ...
        protected virtual void Receive(object sender, EventArgs e)
        {
        ...
        public List<Subscriber> SubscribersOf(string topic)
        {

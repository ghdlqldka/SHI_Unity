...
namespace BioIK {
	...
	public class Evolution {		
		protected Model Model;                 		    		//Reference to the kinematic model
		protected int PopulationSize;                 			//Number of individuals (population size)
		protected int Elites;                                   //Number of elite individuals
        protected int Dimensionality;                  			//Search space dimensionality

		protected double[] LowerBounds;                         //Constraints for the lower bounds
        protected double[] UpperBounds; 		                    //Constraints for the upper bounds

		protected Individual[] Population;                      //Array for current individuals
        protected Individual[] Offspring;							//Array for offspring individuals

		protected List<Individual> Pool = new List<Individual>();	//Selection pool for recombination
		protected int PoolCount;									//Current size of the selection pool
		protected double[] Probabilities;							//Current probabilities for selection
		protected double Gene;									//Simple storage variable #1
		protected double Weight;                                    //Simple storage variable #2
        protected bool[] Constrained;

        //Variables for elitism exploitation
        protected bool UseThreading;
        protected bool Evolving = false;
        protected bool[] Improved = null;
        protected Model[] Models = null;
        protected BFGS[] Optimisers = null;

        //Threading
		protected bool ThreadsRunning = false;
        protected ManualResetEvent[] Handles = null;
        protected Thread[] Threads = null;
        protected bool[] Work = null;

        //Variables for optimisation queries
        protected double[] Solution;                            //Evolutionary solution
        protected double Fitness;                  				//Evolutionary fitness
		...
		public virtual double[] Optimise(int generations, double[] seed) {
            if(UseThreading) {
                for(int i=0; i<Elites; i++) {
                    if(!Threads[i].IsAlive) {
                        Threads[i].Start();
		...
		protected void Initialise(double[] seed) {
			for(int i=0; i<Dimensionality; i++) {
        ...
        protected virtual void SurviveThread(int index) {
		...
		protected virtual void SurviveSequential(int index, double timeout) {
            //Copy elitist survivor
            Individual survivor = Population[index];
            Individual elite = Offspring[index];
            for(int i=0; i<Dimensionality; i++) {

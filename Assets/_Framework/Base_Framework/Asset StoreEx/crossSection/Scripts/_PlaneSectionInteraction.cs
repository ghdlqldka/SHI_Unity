...
{
    public class _PlaneSectionInteraction : PlaneSectionInteraction
    {
        private static string LOG_FORMAT = "<color=#00F138><b>[_PlaneSectionInteraction]</b></color> {0}";
        ...
        protected override void Update()
        {
            ...
            {
                Debug.LogFormat(LOG_FORMAT, "Input.GetMouseButtonDown(0)");
                ...
                if (hits.Length > 0)
                {
                    ...
                    RaycastHit[] backHits = Physics.RaycastAll(backRay, raycastBackDistance - _Camera.nearClipPlane);

                    //recalculate distances in relation to the camera
                    for (int i = 0; i < backHits.Length; i++)
                        backHits[i].distance = raycastBackDistance - backHits[i].distance;
                    ...
                    for (int i = 0; i < hitList.Count; i++)
                    {
                        ...
                        if (Array.IndexOf(objectColliders, hitList[i].collider) == -1)
                            continue;
                        //

...
namespace _Base_Framework //ZenFulcrum.EmbeddedBrowser
{
	/**
	 * Getting CEF running on a build result requires some fiddling to get all the files in the right place.
	 */
	class _Base_Framework_PostProcessBuild //PostBuildStandalone
	{
		private static string LOG_FORMAT = "<color=magenta><b>[_Base_Framework_PostProcessBuild]</b></color> {0}";
		...
		*/
		static readonly List<string> __copyFileList__ = new List<string>(){"Base_Framework_AppConfig.ini"};
		...
		public static void PostprocessBuild(BuildTarget target, string buildFile)
		{
			try
			{
				if (_Base_Framework_Config.Product != _Base_Framework._Base_Framework_Config._Product.Base_Framework)
				{
					Debug.LogWarningFormat(LOG_FORMAT, "This is NOT <b><color=red>Base_Framework</color></b>!!!!!");
					return;
				}
			...
			{
				EditorUtility.DisplayDialog("Base_Framework build processing failed", ex.Message, "OK");
				throw;
			}
		}
		...
		private static string PostProcessBuild_Folder
		{
			get
			{
				// path : D:\Works\Base_Framework\Assets\_Framework\Base_Framework\_PostProcessBuild\Editor\_Base_Framework_PostProcessBuild.cs

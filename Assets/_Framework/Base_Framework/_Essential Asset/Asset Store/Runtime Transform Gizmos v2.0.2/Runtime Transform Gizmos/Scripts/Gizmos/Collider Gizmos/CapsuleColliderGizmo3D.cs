    ...
    public class CapsuleColliderGizmo3D : GizmoBehaviour
    {
        ...
        public override void OnGizmoAttemptHandleDragBegin(int handleId)
        {
            ...
            if (OwnsHandle(handleId))
            {
                ...
                Vector3 right = _targetCollider.transform.rotation * rotationByDir * Vector3.right;
                Vector3 up = _targetCollider.transform.rotation * rotationByDir * Vector3.up;
                Vector3 look = _targetCollider.transform.rotation * rotationByDir * Vector3.forward;
        ...
        public override void OnGizmoRender(Camera camera)
        {
            ...
            Quaternion colliderRotation = _targetCollider.transform.rotation;
            Vector3 centralAxis = colliderRotation * Vector3.up;
            if (_targetCollider.direction == 0) centralAxis = colliderRotation * Vector3.right;
            else if (_targetCollider.direction == 2) centralAxis = colliderRotation * Vector3.forward;
            ...
            if (lineHeight > 0.0f)
            {
                Vector3 right = _targetCollider.transform.right;
                Vector3 look = _targetCollider.transform.forward;
                if (_targetCollider.direction == 0)
                {
                    right = _targetCollider.transform.up;
                    look = _targetCollider.transform.forward;
                }
                else if (_targetCollider.direction == 2)
                {
                    right = _targetCollider.transform.right;
                    look = _targetCollider.transform.up;
                }

                GLRenderer.DrawLine3D(worldCenter - right * worldRadius - centralAxis * lineHeight * 0.5f, worldCenter - right * worldRadius + centralAxis * lineHeight * 0.5f);
                GLRenderer.DrawLine3D(worldCenter + right * worldRadius - centralAxis * lineHeight * 0.5f, worldCenter + right * worldRadius + centralAxis * lineHeight * 0.5f);
                GLRenderer.DrawLine3D(worldCenter - look * worldRadius - centralAxis * lineHeight * 0.5f, worldCenter - look * worldRadius + centralAxis * lineHeight * 0.5f);
                GLRenderer.DrawLine3D(worldCenter + look * worldRadius - centralAxis * lineHeight * 0.5f, worldCenter + look * worldRadius + centralAxis * lineHeight * 0.5f);
            }
            ...
            Vector3 semiCircleCenter = worldCenter + centralAxis * (worldHeight * 0.5f - worldRadius);
            GL.PushMatrix();
            GL.MultMatrix(Matrix4x4.TRS(semiCircleCenter, colliderRotation * rotationByDir, circleScale));
            GLRenderer.DrawLines3D(_semiCirclePts);
            GL.PopMatrix();

            GL.PushMatrix();
            GL.MultMatrix(Matrix4x4.TRS(semiCircleCenter, colliderRotation * rotationByDir * Quaternion.Euler(0.0f, 90.0f, 0.0f), circleScale));
            GLRenderer.DrawLines3D(_semiCirclePts);
            GL.PopMatrix();

            GL.PushMatrix();
            GL.MultMatrix(Matrix4x4.TRS(semiCircleCenter, colliderRotation * rotationByDir * Quaternion.Euler(90.0f, 0.0f, 0.0f), new Vector3(worldRadius, worldRadius, worldRadius)));
            GLRenderer.DrawLines3D(_circlePts);
            GL.PopMatrix();

            // Bottom semi-circles and cap circle
            semiCircleCenter = worldCenter - centralAxis * (worldHeight * 0.5f - worldRadius);
            GL.PushMatrix();
            GL.MultMatrix(Matrix4x4.TRS(semiCircleCenter, colliderRotation * rotationByDir * Quaternion.Euler(0.0f, 0.0f, 180.0f), circleScale));
            GLRenderer.DrawLines3D(_semiCirclePts);
            GL.PopMatrix();

            GL.PushMatrix();
            GL.MultMatrix(Matrix4x4.TRS(semiCircleCenter, colliderRotation * rotationByDir * Quaternion.Euler(0.0f, 90.0f, 0.0f) * Quaternion.Euler(0.0f, 0.0f, 180.0f), circleScale));
            GLRenderer.DrawLines3D(_semiCirclePts);
            GL.PopMatrix();

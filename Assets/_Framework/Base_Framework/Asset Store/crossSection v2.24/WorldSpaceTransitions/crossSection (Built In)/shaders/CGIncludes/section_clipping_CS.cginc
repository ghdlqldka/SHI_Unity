...
//Plane clipping definitions
...
#if CLIP_PLANE || CLIP_PIE || CLIP_SPHERE || CLIP_TUBES || CLIP_TUBE || CLIP_BOX || CLIP_CORNER || CLIP_SPHERES || CLIP_PRISM || CLIP_PRISMS || CLIP_CYLINDER || CLIP_CYLINDERS || CLIP_TETRA || CLIP_TETRAS || CLIP_CONE || CLIP_CONES || CLIP_ELLIPSOID || CLIP_ELLIPSOIDS || CLIP_CUBOID || CLIP_CUBOIDS || CLIP_NONE
...
#if CLIP_PLANE || CLIP_SPHERE || CLIP_TUBE 
...
#if CLIP_BOX ||  CLIP_CORNER ||  CLIP_PRISM || CLIP_CYLINDER || CLIP_TETRA || CLIP_CONE || CLIP_ELLIPSOID || CLIP_CUBOID || CLIP_PIE
	...
	// pieIntersect - ray intersects the pie
	// ro is the ray origin in world space
	// rd is the ray direction in world space
	// txx is the world-to-box transformation
	// angle is the pie opening angle
	bool pieIntersect(in float3 ro, in float3 rd, in float4x4 txx, in float angle)
	{
		float3 rdd = (mul(txx, float4(rd, 0.0))).xyz;
		float3 roo = (mul(txx, float4(ro, 1.0))).xyz;
		...
		if(angle%(2*PI)<=PI)
		{
			return !(poo.z < poo.x * tan(angle/2) && poo.z > - poo.x * tan(angle/2));
		}
		else
		{
			float3 campos = (mul(txx, float4(_WorldSpaceCameraPos, 1.0))).xyz;
			/*
			//cut backfaces only
			if((campos.z>campos.x*tan(angle/2))&&(campos.z<-campos.x*tan(angle/2)))	return false;
			if((campos.z<campos.x*tan(angle/2))&&(campos.z>-campos.x*tan(angle/2)))	return false;
			if(campos.z<0)
			{
				if((cross(-campos, rdd).y>0)&&(poo.z>poo.x *tan(angle/2))) return true;
			}
			else
			{
				if((cross(-campos, rdd).y<0)&&(poo.z<-poo.x *tan(angle/2))) return true;
			...
			*/
			bool val = false;
			val = ((poo.z < poo.x * tan(angle/2))||(cross(-campos, rdd).y>0&&campos.z<-campos.x*tan(angle/2)&&campos.z<0));
			val = val && (poo.z > - poo.x * tan(angle/2)||(cross(-campos, rdd).y<0&&campos.z>0&&campos.z>campos.x*tan(angle/2)));
			return val;
		}
	}

	bool clipPie(in float3 po, in float4x4 txx, in float angle)
	{
		float3 poo = (mul(txx, float4(po, 1.0))).xyz;
		bool clip = false;
		//angle = angle%(2*PI);
		if(angle%(2*PI)<=PI)
		{
			clip = (poo.z < poo.x * tan(angle/2) && poo.z > - poo.x * tan(angle/2));
		}
		else
		{
			clip = (poo.z > poo.x * tan(angle/2) || poo.z < - poo.x * tan(angle/2));
	...
	bool ClipBool(in float3 posWorld) {
		bool _clip = false;
#if CLIP_PIE
		bool _clipPie = clipPie(posWorld, _WorldToObjectMatrix, _pieAngle);
		_clip = _clip || _clipPie;
#endif
#if CLIP_PLANE
...
#endif
#if CLIP_PIE
		_clip = !pieIntersect(posWorld, normalize(_WorldSpaceCameraPos - posWorld), _WorldToObjectMatrix, _pieAngle);
		if (_clip) discard;
#endif
		if (_clip) discard;

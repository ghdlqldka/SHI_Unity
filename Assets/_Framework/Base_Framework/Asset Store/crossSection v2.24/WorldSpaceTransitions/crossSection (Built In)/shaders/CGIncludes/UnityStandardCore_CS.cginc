...
#if UNITY_REQUIRE_FRAG_WORLDPOS || SECTION_CLIPPING_ENABLED
...
fixed f = 1;
	...
	#if UNITY_PACK_WORLDPOS_WITH_TANGENT
	SECTION_CLIP(float3(i.tangentToWorldAndPackedData[0].w, i.tangentToWorldAndPackedData[1].w, i.tangentToWorldAndPackedData[2].w))
	#else
	SECTION_CLIP(s.posWorld)
	#endif
	...
	bool showSectionColor = false;
	#if USE_SECTION_COLOR && SECTION_CLIPPING_ENABLED
	showSectionColor = (f*(1-2*_negativeScale)<0);
	#endif
	c.rgb = showSectionColor ? _SectionColor.rgb : c.rgb;
...
)
{
	...
	bool showSectionColor = false;
	#if USE_SECTION_COLOR && SECTION_CLIPPING_ENABLED
	showSectionColor = (facing*(1-2*_negativeScale)<0);
	#endif

	if(showSectionColor) emissiveColor = float4(1,1,1,1) -_SectionColor;

	UnityStandardData data;
	data.diffuseColor = showSectionColor? _SectionColor : s.diffColor ;
	data.occlusion = showSectionColor? float4(0,0,0,1) : occlusion;
	data.specularColor = showSectionColor? float4(0,0,0,1) : s.specColor;
	data.smoothness = showSectionColor? float4(0,0,0,1) : s.smoothness;
	data.normalWorld = showSectionColor? float4(0,0,0,1) : s.normalWorld;

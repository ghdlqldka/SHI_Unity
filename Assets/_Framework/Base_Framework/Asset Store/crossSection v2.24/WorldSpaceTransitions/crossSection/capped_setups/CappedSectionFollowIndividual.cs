//The purpose of this script is to setup and update the global shader properties for the capped sections 

using UnityEngine;
using System;
using System.Text.RegularExpressions;
using System.Collections.Generic;

namespace WorldSpaceTransitions
{
    //[ExecuteInEditMode]
    public class CappedSectionFollowIndividual : MonoBehaviour
    {

        private enum Mode { box, corner };
        private Mode sectionMode;

        private Vector3 tempPos;
        private Vector3 tempScale;
        private Quaternion tempRot;

        private List<Material> instancedMaterials;
        private List<Material> sharedMaterials;

        private List<Material> newCapMaterials;
        private List<Material> sharedCapMaterials;

        public bool followPosition = true;
        //public bool followRotation = true;

        void Awake()
        {
            if (gameObject.GetComponent<CappedSectionBox>()) sectionMode = Mode.box;
            if (gameObject.GetComponent<CappedSectionCorner>()) sectionMode = Mode.corner;
        }
        void Start()
        {
            string pattern = @"\d*$";
            Match m = Regex.Match(gameObject.name, pattern, RegexOptions.IgnoreCase);
            if (m.Success)
                Debug.Log("Found '{0}' at position {1}."+ m.Value);
            int stencilValue = int.Parse(m.ToString()) + 1;
            //onsole.WriteLine("Found '{0}' at position {1}.", m.Value, m.Index);

            newCapMaterials = new List<Material>();
            sharedCapMaterials = new List<Material>();
            
            Renderer[] boxrenderers = GetComponentsInChildren<Renderer>();
            foreach (Renderer rend in boxrenderers)
            {
                Debug.Log(gameObject.name + " | " + rend.name);
                Material[] mats = rend.sharedMaterials;
                Material[] newMats = new Material[mats.Length];
                for (int i = 0; i < mats.Length; i++)
                {
                    int ind = sharedCapMaterials.IndexOf(mats[i]);
                    if (ind == -1)
                    {
                        ind = sharedCapMaterials.Count;
                        sharedCapMaterials.Add(mats[i]);
                        Material newMaterial;
                        if (mats[i].HasProperty("_StencilMask"))
                        {
                            Debug.Log("has");
                            newMaterial = new Material(mats[i]);
                            newMaterial.SetInt("_StencilMask", stencilValue);
                            newMaterial.name = gameObject.name + ind.ToString();
                        }
                        else
                        {
                            newMaterial = mats[i];
                        }
                        newCapMaterials.Add(newMaterial);
                    }
                    newMats[i] = newCapMaterials[ind];
                }
                rend.materials = newMats;
            }
            
            instancedMaterials = new List<Material>();
            sharedMaterials = new List<Material>();
            SectionSetup ss = GetComponent<SectionSetup>();
            if (ss)
            {
                GameObject boxmodel = ss.model;
                Renderer[] allrenderers = boxmodel.GetComponentsInChildren<Renderer>();
                foreach(Renderer rend in allrenderers)
                {
                    Material[] mats = rend.sharedMaterials;
                    Material[] newMats = new Material[mats.Length];
                    for (int i=0; i<mats.Length; i++)
                    {
                        int ind = sharedMaterials.IndexOf(mats[i]);
                        if (ind==-1)
                        {
                            ind = sharedMaterials.Count;
                            sharedMaterials.Add(mats[i]);
                            Material newMaterial = new Material(mats[i]);
                            newMaterial.SetInt("_StencilMask", stencilValue);
                            string str = gameObject.name + ind.ToString();
                            //Debug.Log(str);
                            newMaterial.name = str;

                            instancedMaterials.Add(newMaterial);
                        }
                        newMats[i] = instancedMaterials[ind];
                    }
                    rend.materials = newMats;
                }
            }
#if UNITY_EDITOR
            if (!Application.isPlaying) return;
#endif
            Shader.SetGlobalColor("_SectionColor", Color.black);
            Matrix4x4 mx = Matrix4x4.TRS(transform.position, transform.rotation, Vector3.one);
            Shader.SetGlobalMatrix("_WorldToObjectMatrix", mx.inverse);
            SetSection();
        }

        void LateUpdate()
        {
#if UNITY_EDITOR
            if (!Application.isPlaying) return;
#endif
            if (tempPos != transform.position || tempRot != transform.rotation || tempScale != transform.localScale)
            {

                tempPos = transform.position;
                tempRot = transform.rotation;
                tempScale = transform.localScale;
                SetSection();
            }
            Matrix4x4 mx = Matrix4x4.TRS(transform.position, transform.rotation, Vector3.one);
            foreach (Material m in instancedMaterials)
            {
                m.SetMatrix("_WorldToObjectMatrix", mx.inverse);
                m.SetVector("_SectionScale", transform.localScale);
            }
            return;
#pragma warning disable 0162
            Shader.SetGlobalMatrix("_WorldToObjectMatrix", mx.inverse);
#pragma warning restore 0162
        }


        void OnDisable()
        {

            Shader.DisableKeyword("CLIP_BOX");
            //Shader.SetGlobalInt("_CLIP_BOX", 0);
            Shader.DisableKeyword("CLIP_CORNER");
            //Shader.SetGlobalInt("_CLIP_CORNER", 0);
            Shader.EnableKeyword("CLIP_NONE");
            Shader.EnableKeyword("__");
        }

        void OnEnable()
        {
#if UNITY_EDITOR
            if (!Application.isPlaying)
            {
                Shader.EnableKeyword("CLIP_NONE");
                Shader.EnableKeyword("__");
                return;
            }
#endif

            if (sectionMode == Mode.box)
            {
                Shader.DisableKeyword("CLIP_NONE");
                Shader.EnableKeyword("CLIP_BOX");
                //Shader.SetGlobalInt("_CLIP_BOX", 1);
            }
            if (sectionMode == Mode.corner)
            {
                Shader.DisableKeyword("CLIP_NONE");
                Shader.EnableKeyword("CLIP_CORNER");
                //Shader.SetGlobalInt("_CLIP_CORNER", 1);
            }
            //SetSection();
        }


        void OnApplicationQuit()
        {
            Shader.DisableKeyword("CLIP_BOX");
            //Shader.SetGlobalInt("_CLIP_BOX", 0);
            Shader.DisableKeyword("CLIP_CORNER");
            //Shader.SetGlobalInt("_CLIP_CORNER", 0);
            Shader.EnableKeyword("CLIP_NONE");
        }

        void SetSection()
        {
            if (followPosition)
            {
                Matrix4x4 mx = Matrix4x4.TRS(transform.position, transform.rotation, Vector3.one);
                foreach (Material m in instancedMaterials)
                {
                    m.SetMatrix("_WorldToObjectMatrix", mx.inverse);
                    m.SetVector("_SectionScale", transform.localScale);
                }
                return;
#pragma warning disable 0162
                Shader.SetGlobalMatrix("_WorldToObjectMatrix", mx.inverse);
#pragma warning restore 0162
                Shader.SetGlobalVector("_SectionScale", transform.localScale);
            }
        }

    }
}
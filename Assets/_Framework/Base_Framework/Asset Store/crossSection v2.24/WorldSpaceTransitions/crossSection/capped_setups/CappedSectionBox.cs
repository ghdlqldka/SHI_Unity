...
namespace WorldSpaceTransitions
{
    public class CappedSectionBox : MonoBehaviour, ISizedSection
    {
        ...
        protected enum GizmoAxis { X, Y, Z, Xneg, Yneg, Zneg, XYRotate, XZRotate, YZRotate, none };
        protected GizmoAxis selectedAxis;

        private RaycastHit hit;
        protected Ray ray, ray1;
        protected Plane dragplane;
        protected float rayDistance, newRotY, rayDistancePrev, distance;
        protected Vector3 lookCamera, startDrag, startPos, startDragRot, lookHitPoint, startScale;
                        ...
                        case GizmoAxis.X:
                            {
                                ...
                                transform.position = startPos + 0.5f * (projectedTranslation.normalized * projectedTranslation.magnitude);
                                lsx += projectedTranslation.magnitude * Mathf.Sign(Vector3.Dot(projectedTranslation, transform.right));
                                break;
                            }
                        case GizmoAxis.Xneg:
                            {
                        ...
                        case GizmoAxis.Y:
                            {
                                ...
                                transform.position = startPos + 0.5f * (projectedTranslation.normalized * projectedTranslation.magnitude);
                                lsy += projectedTranslation.magnitude * Mathf.Sign(Vector3.Dot(projectedTranslation, transform.up));
                                break;
                            }
                        case GizmoAxis.Yneg:
                            {
                        ...
                        case GizmoAxis.Z:
                            {
                                ...
                                transform.position = startPos + 0.5f * (projectedTranslation.normalized * projectedTranslation.magnitude);
                                lsz += projectedTranslation.magnitude * Mathf.Sign(Vector3.Dot(projectedTranslation, transform.forward));
                                break;
                            }
                        case GizmoAxis.Zneg:
                            {
...
//#if ENABLE_LEGACY_INPUT_MANAGER
        ...
        public virtual void Size(Bounds b, GameObject g, BoundsOrientation orientation)
        {

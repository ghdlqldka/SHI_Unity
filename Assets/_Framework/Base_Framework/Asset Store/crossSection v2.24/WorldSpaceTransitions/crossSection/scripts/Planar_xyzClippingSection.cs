...
namespace WorldSpaceTransitions.Examples
{
    ...
    public class Planar_xyzClippingSection : MonoBehaviour, ISizedSection
    {
        ...
        [HideInInspector]
        public bool cappedSectionMode = false;
        ...
        void Start()
        {
            ...
            SliderSetup();
            Shader.SetGlobalVector("_SectionPoint", new Vector3(bounds.center.x, slider.value + zeroReferenceVector.y, bounds.center.z));
            setupGizmo();
            setSection();
        }
        ...
        {
            if (middleSliderLabel) middleSliderLabel.text = value.ToString("G4");
        ...
        public void SetAxis(bool b, ConstrainedAxis a)
        {
            if (b)
            {
                ...
                if (rectGizmo)
                {
                    ...
                    rg.SetSizedGizmo(bounds.size, selectedAxis, gizmoOn);
                }
                setSection();
            }
        }
        ...
        void setSection()
        {
            ...
            switch (selectedAxis)
            {
                case ConstrainedAxis.X:
                    ...
                    {
                        sectionpointLocal.x = Mathf.Clamp(sectionpointLocal.x, Min.x - 0.01f, Min.x + sliderRange.x);
                        sliderVal = sectionpointLocal.x - zeroReferenceVector.x;
                    }
                    else
                    {
                        sectionX = Mathf.Clamp(sectionX, bounds.min.x - 0.01f, bounds.min.x + sliderRange.x);
                        sliderVal = sectionX - zeroReferenceVector.x;
                ...
                case ConstrainedAxis.Y:
                    ...
                    if (boundsMode == BoundsOrientation.objectOriented)
                    {
                        sectionpointLocal.y = Mathf.Clamp(sectionpointLocal.y, Min.y - 0.01f, Min.y + sliderRange.y);
                        sliderVal = sectionpointLocal.y - zeroReferenceVector.y;
                    }
                    else
                    {
                        sectionY = Mathf.Clamp(sectionY, bounds.min.y - 0.01f, bounds.min.y + +sliderRange.y);
                        sliderVal = sectionY - zeroReferenceVector.y;
                ...
                case ConstrainedAxis.Z:
                    ...
                    if (boundsMode == BoundsOrientation.objectOriented)
                    {
                        sectionpointLocal.z = Mathf.Clamp(sectionpointLocal.z, Min.z - 0.01f, Min.z + sliderRange.z);
                        sliderVal = sectionpointLocal.z - zeroReferenceVector.z;
                    }
                    else
                    {
                        sectionZ = Mathf.Clamp(sectionZ, bounds.min.z - 0.01f, bounds.min.z + sliderRange.z);
                        sliderVal = sectionZ - zeroReferenceVector.z;
            ...
            if (boundsMode == BoundsOrientation.objectOriented)
            {
                ...
                if(rectGizmo) rectGizmo.transform.position = wpos;
            }
            else
            {
                if (rectGizmo) rectGizmo.transform.position = sectionpoint;
            }
            ...
            if (topSliderLabel) topSliderLabel.text = sliderMaxVal.ToString("G4");
            if (bottomSliderLabel) bottomSliderLabel.text = sliderMinVal.ToString("G4");
            ...
            if (slider)
            {
                ...
                middleSliderLabel.text = sliderVal.ToString("G4");
        ...
        void setupGizmo()
        {
            ...
            if (rectGizmo)
            {
                ...
                rectGizmo.transform.position = bounds.center + (-bounds.extents.y + (slider ? slider.value : 0) + zeroReferenceVector.y) * transform.up;
                rectGizmo.transform.rotation = _rot;
                ...
                rectGizmo.GetComponent<Renderer>().enabled = gizmoOn;
                rectGizmo.SetActive(true);
            }
        }
        ...
        void Update()
        {
           ...
           if(Mouse.current.leftButton.isPressed)
...
#endif
            {
                ...
                if (!model) return;
                if (!rectGizmo) return;
                Collider coll = rectGizmo.GetComponent<Collider>();
                if (!coll) return;
                RectGizmo rg = rectGizmo.GetComponent<RectGizmo>();
                if (coll.Raycast(ray, out hit, 10000f))
                {
                    if (gizmoOn) rectGizmo.GetComponent<Renderer>().enabled = true;
                    StartCoroutine(dragGizmo(rg));
                }
                else
                {
                    rg.SetSizedGizmo(bounds.size, selectedAxis, false);
                }
            }
        }

        IEnumerator dragGizmo(RectGizmo rg)
        {
            rg.SetSizedGizmo(bounds.size, selectedAxis, true);
